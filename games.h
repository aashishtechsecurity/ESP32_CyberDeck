...
// Snake game settings
const int GRID_SIZE = 4; // 4x4 pixel cells
const int GRID_WIDTH = SCREEN_WIDTH / GRID_SIZE;  // 32
const int GRID_HEIGHT = SCREEN_HEIGHT / GRID_SIZE; // 16
...
void updateSnake(JoyDirection input) {
  ...
  if (nextX < 0 || nextX >= GRID_WIDTH || nextY < 2 || nextY >= GRID_HEIGHT) {
    gameOver = true;
    setRGBColor(255, 0, 0); // Flash RED on death
    delay(400);
    setRGBColor(0, 0, 0);
    return;
  }
